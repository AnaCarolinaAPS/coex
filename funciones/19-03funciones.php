<?php
	...
	function getProdbyCategoria ($categoria, $offset, $limit) {
		...
		if($categoria != 'ALL') {
			...
			$sql = "SELECT tb_producto.* FROM tb_producto WHERE tb_producto.activo = 1 AND tb_producto.id_categoria IN (SELECT id FROM tb_categoria WHERE tb_categoria.activo = 1) ORDER BY nombre ASC LIMIT $offset, $limit";
		}
		$query = $connection->prepare($sql);
		$query->execute();
